...
namespace Terraria.ModLoader
{
	public static class Logging
	{
		...
		internal static ILog Tea { get; } = LogManager.GetLogger("Tea");
		...
		internal static void Init() {
			...
			Tea.InfoFormat("Starting {0} {1} {2} ({3})", ModLoader.VersionName, Platform.Current.Type, side, DateTime.Now.ToString("d"));
			Tea.InfoFormat("Executable: {0}", Assembly.GetEntryAssembly().Location);
			Tea.InfoFormat("Working Directory: {0}", Path.GetFullPath(Directory.GetCurrentDirectory()));
			Tea.InfoFormat("Launch Parameters: {0}", string.Join(" ", Program.LaunchParameters.Select(p => (p.Key + " " + p.Value).Trim())));

			foreach (var line in initWarnings)
				Tea.Warn(line);

			AppDomain.CurrentDomain.UnhandledException += (s, args) => Tea.Error("Unhandled Exception", args.ExceptionObject as Exception);
		...
		private static void LogFirstChanceExceptions() => AppDomain.CurrentDomain.FirstChanceException += FirstChanceExceptionHandler;
		...
		private static void FirstChanceExceptionHandler(object sender, FirstChanceExceptionEventArgs args) {
			...
			try {
				...
				Console.ResetColor();
#endif
				Tea.Warn(Language.GetTextValue("tModLoader.RuntimeErrorSilentlyCaughtException") + '\n' + exString);
				...
				Tea.Warn("FirstChanceExceptionHandler exception", e);
			}
			finally {
		...
		public static void PrettifyStackTraceSources(StackFrame[] frames) {
			...
			foreach (var frame in frames) {
				...
				int idx = filename.LastIndexOf(assembly.GetName().Name, StringComparison.InvariantCultureIgnoreCase);
				if (idx > 0) {
